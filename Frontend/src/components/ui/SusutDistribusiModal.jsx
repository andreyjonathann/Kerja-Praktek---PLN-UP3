...
export default function SusutDistribusiModal({
  open,
  onOpenChange,
  rowData,
  year,
    ...
    if (open && rowData) {
      if (rowData.id) {
        ...
        setFormData({
          ...
          kwh_netto: rowData.kwh_netto ?? '',
          pssd: rowData.pssd ?? '',
          kwh_jual_309: rowData.kwh_jual_309 ?? '',
  ...
  const judul = rowData.id ? `Detail Susut Distribusi — ${bulanName} ${tahun}` : `Tambah Data Susut Distribusi — ${bulanName} ${tahun}`;
  ...
  const handleSave = async () => {
    if (!formData.bulan || !formData.kwh_netto || formData.kwh_netto <= 0 || formData.pssd === '' || formData.kwh_jual_309 === '') {
      Swal.fire('Error', 'Bulan, KWh Netto, PSSD, dan KWh Jual 309 harus diisi. KWh Netto tidak boleh 0.', 'error');
      return;
    }
    ...
    try {
      const payload = {
        ...
        kwh_netto: parseFloat(formData.kwh_netto),
        pssd: parseFloat(formData.pssd),
        kwh_jual_309: parseFloat(formData.kwh_jual_309),
        keterangan: formData.keterangan
      };

      if (rowData.id) {
        await api.put(`/v1/susut-distribusi/${rowData.id}`, payload);
        Swal.fire({ icon: 'success', title: 'Berhasil', text: 'Data berhasil diupdate', timer: 1500, showConfirmButton: false });
      } else {
        await api.post('/v1/susut-distribusi', payload);
  ...
  // Kalkulasi live persen
  const net = parseFloat(formData.kwh_netto) || 0;
  const pssd = parseFloat(formData.pssd) || 0;
  const jual = parseFloat(formData.kwh_jual_309) || 0;
  const calculatedPersen = net > 0 ? (((net - pssd - jual) / net) * 100).toFixed(4) : '0.0000';
  
  const fmt = (v) => v != null ? Number(v).toLocaleString('id-ID') : '—';
  const fmtPersen = (v) => v != null ? `${Number(v).toFixed(4)}%` : '—';
      ...
      >
        ...
        {rowData.id && !showEditForm && (
          <div style={{ marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid #f3f4f6' }}>
              <span style={{ fontWeight: 600, fontSize: 14, color: '#0f172a' }}>KWh Jual 309 (Tanpa Emin)</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div>
                ...
                <input 
                  ...
                  step="0.01"
                  min="0.01"
                  value={formData.kwh_netto}
                  onChange={(e) => setFormData({...formData, kwh_netto: e.target.value})}
                  style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '8px 12px', fontSize: 14, outline: 'none' }} 
                  placeholder="Masukkan KWh Netto"
                />
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  min="0"
                  value={formData.pssd}
                  onChange={(e) => setFormData({...formData, pssd: e.target.value})}
                  style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '8px 12px', fontSize: 14, outline: 'none' }} 
                  placeholder="Masukkan PSSD"
                />
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
                  KWh Jual 309 (Tanpa Emin)
                </label>
                <input 
                  type="number" 
                  step="0.01"
                  min="0"
                  value={formData.kwh_jual_309}
                  onChange={(e) => setFormData({...formData, kwh_jual_309: e.target.value})}
                  style={{ width: '100%', border: '1px solid #e2e8f0', borderRadius: 8, padding: '8px 12px', fontSize: 14, outline: 'none' }} 
                  placeholder="Masukkan KWh Jual 309"
                />
              </div>

              <div>
                <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
                  Realisasi Susut % (Dihitung Otomatis)
                </label>
                <input 
                  type="text" 
                  value={`${calculatedPersen}%`}
                  disabled
...
function DeleteConfirmButton({ id, bulanName, onSuccess }) {
  ...
  const confirmDelete = async () => {
    setLoading(true);
    try {
      await api.delete(`/v1/susut-distribusi/${id}`);
      onSuccess();
    } catch (err) {
