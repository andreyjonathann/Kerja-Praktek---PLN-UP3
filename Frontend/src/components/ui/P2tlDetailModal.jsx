...
export default function P2tlDetailModal({
  open,
  onOpenChange,
  rowData,
  year,
  ...
  const judul     = `Perolehan kWh P2TL — ${bulanName} ${tahun}`
  ...
  const handleEdit = () => {
    navigate(`/p2tl/edit/${bulanNum}/${tahun}`)
    onOpenChange(false)
  }
  ...
  const handleDelete = async () => {
    setIsDeleting(true)
    try {
      await api.delete(`/v1/p2tl/${rowData.id}`)
      setIsDeleting(false)
      onOpenChange(false)
      ...
      toast.textContent = `Data P2TL ${bulanName} ${tahun} berhasil dihapus.`
      toast.style.cssText = `
      ...
      >
        ...
        {hasData ? (
              ...
              showConfirm ? (
                ...
                >
                  <p style={{ fontSize: 14, color: '#991b1b', fontWeight: 600, marginBottom: 14, lineHeight: 1.5 }}>
                    Hapus data P2TL {bulanName} {tahun}?
          ...
          <div style={{ textAlign: 'center', padding: '24px 0 8px' }}>
            ...
            {!isViewer && (
              <button
                onClick={() => { navigate('/p2tl/input'); onOpenChange(false) }}
                style={{
