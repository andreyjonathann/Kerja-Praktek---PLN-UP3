...
export default function DayaTersambungPage() {
  ...
  const handleExport = () => {
    ...
    exportToExcel(data.map(d => ({
      Bulan: d.label, Target: d.daya_target, Realisasi: d.daya_total ?? '',
      ...Object.fromEntries(TARIF_KEYS.map(k => [`Tarif ${k.toUpperCase()}`, d[`daya_${k}`] ?? ''])),
    })), `Daya_Tersambung_${filters.year}`)
  }

  const tableColumns = [
    { key:'label',  label:'Bulan', width:'72px', align:'center' },
    { key:tgtKey,   label:'Target (kVA)',    align:'right', render: v => v != null ? formatNumber(v) : '—' },
    { key:chartKey, label:'Realisasi (kVA)', align:'right', render: (v, row) => v != null
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:24 }} className="animate-fade-in">
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div className="icon-wrapper-interactive" style={{ width:34, height:34, borderRadius:10,
              ...
              border:'1px solid rgba(217,119,6,0.25)', display:'flex', alignItems:'center', justifyContent:'center' }}>
        ...
        <p className="page-description">
          Realisasi penambahan daya tersambung per golongan tarif · Tahun {filters.year}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <ChartWrapper title={tab==='monthly'?'Daya Tersambung Bulanan':'Daya Tersambung Kumulatif'}
        ...
        <ChartWrapper title="Breakdown per Golongan Tarif"
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={filled}>
              ...
              <Tooltip content={<TOOLTIP />} />
              <Legend wrapperStyle={{ fontSize:13, fontWeight:600 }} />
              {TARIF_KEYS.map((k,i) => <Bar key={k} dataKey={`daya_${k}`} name={k.toUpperCase()} stackId="a" fill={CHART_COLORS[i]} />)}
            </BarChart>
          </ResponsiveContainer>
        </ChartWrapper>
      </div>
      ...
      <div className="card p-5">
        <h3 className="section-title mb-4">Detail Data Daya Tersambung {tab==='monthly'?'Bulanan':'Kumulatif'} (kVA)</h3>
