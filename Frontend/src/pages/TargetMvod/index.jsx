...
export default function TargetMvodPage() {
  ...
  const fetchTargets = async () => {
    ...
    try {
      const res = await api.get('/v1/mvod/targets', { params: { tahun: selectedYear } });
      ...
      UP3_LIST.forEach(up3 => {
        ...
        targetsMap[up3] = {
          gi: existing ? existing.sla_gi_menit : 30,
          jtm: existing ? existing.sla_jtm_menit : 60,
          gd: existing ? existing.sla_gd_menit : 90
        };
      });
    ...
    try {
      const payload = {
        ...
        targets: Object.entries(targets).map(([up3, val]) => ({
          up3,
          sla_gi_menit: parseFloat(val.gi) || 30,
          sla_jtm_menit: parseFloat(val.jtm) || 60,
          sla_gd_menit: parseFloat(val.gd) || 90
        }))
      };

      await api.post('/v1/mvod/targets', payload);
      setMessage({ type: 'success', text: 'SLA target berhasil disimpan!' });
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="w-full mx-auto py-6 animate-fade-in">
      <PageHeader 
        title="Kelola SLA MVOD"
        description="Penetapan batas maksimum SLA (dalam menit) per UP3"
        icon={Target}
        iconColor="#F43F5E"
      ...
      <div className="bg-white border border-slate-200 shadow-sm rounded-none overflow-hidden mb-6 relative">
        ...
        <div className="bg-slate-50 border-b border-slate-200 p-4 flex justify-between items-center pl-6">
            <h2 className="font-bold text-slate-700 text-sm">NILAI SLA PER UP3 (MENIT)</h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-xs text-slate-500 uppercase">
                  ...
                  <th className="px-6 py-3 font-bold text-center border-r border-slate-200">RCT GI (mnt)</th>
                  <th className="px-6 py-3 font-bold text-center border-r border-slate-200">RCT JTM (mnt)</th>
                  <th className="px-6 py-3 font-bold text-center">RCT GD (mnt)</th>
                </tr>
              </thead>
              <tbody>
                {UP3_LIST.map((up3) => (
                  <tr key={up3} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-2 border-r border-slate-200">
                      <input 
                        ...
                        step="0.1"
                        min="0"
                        value={targets[up3]?.gi || ''}
                        onChange={(e) => handleTargetChange(up3, 'gi', e.target.value)}
                        className="w-full text-center bg-white border border-slate-300 focus:border-rose-500 focus:ring-rose-500 text-slate-800 rounded-none px-3 py-1.5 font-bold text-sm"
                      />
                    </td>
                    <td className="px-6 py-2 border-r border-slate-200">
                      <input 
                        ...
                        step="0.1"
                        min="0"
                        value={targets[up3]?.jtm || ''}
                        onChange={(e) => handleTargetChange(up3, 'jtm', e.target.value)}
                        className="w-full text-center bg-white border border-slate-300 focus:border-rose-500 focus:ring-rose-500 text-slate-800 rounded-none px-3 py-1.5 font-bold text-sm"
                      />
                    </td>
                    <td className="px-6 py-2">
                      <input 
                        ...
                        step="0.1"
                        min="0"
                        value={targets[up3]?.gd || ''}
                        onChange={(e) => handleTargetChange(up3, 'gd', e.target.value)}
                        className="w-full text-center bg-white border border-slate-300 focus:border-rose-500 focus:ring-rose-500 text-slate-800 rounded-none px-3 py-1.5 font-bold text-sm"
                      />
                    </td>
                  </tr>
      ...
      <div className="bg-white border border-slate-200 p-4 shadow-sm w-full">
         ...
         >
             {saving ? (
                 ...
                 <><Save size={20} /> SIMPAN SEMUA TARGET SLA</>
             )}
         </button>
      </div>
    </div>
