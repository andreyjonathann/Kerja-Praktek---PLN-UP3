...
export default function TindakLanjutLbkbPage() {
  ...
  const chartKey = tab === 'monthly' ? 'lbkb_real' : 'c_lbkb_real'
  const tgtKey = tab === 'monthly' ? 'lbkb_target' : 'c_lbkb_target'

  const prevLastRow = filled[filled.length - 2]
  const trend = prevLastRow?.lbkb_real
    ? ((lastReal - prevLastRow.lbkb_real) / prevLastRow.lbkb_real) * 100
    : null

  const tableColumns = [
    { key: 'label', label: 'Bulan', width: '72px', align: 'center' },
    { key: tgtKey, label: 'Target (Laporan)', align: 'right', render: v => v != null ? formatNumber(v) : '—' },
    {
      key: chartKey, label: 'Realisasi (Laporan)', align: 'right', render: (v, row) => v != null
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} className="animate-fade-in">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <h1 className="page-heading">Tindak Lanjut LBKB</h1>
        </div>
        <p className="page-description">Realisasi laporan tindak lanjut Lembar Binaan Kontrol/Koreksi Billman (LBKB) · Tahun {filters.year}</p>
      </div>

      {/* ── KPI Cards ─────────────────────────────────────────────────── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        <KpiCard title="Realisasi YTD" value={formatNumber(ytdReal)} unit="Laporan" icon={Activity} color="cyan" achievement={ach} loading={loading} />
        <KpiCard title="Target YTD" value={formatNumber(ytdTgt)} unit="Laporan" icon={Activity} color="blue" loading={loading} />
        <KpiCard title="Bulan Terakhir" value={formatNumber(lastReal)} unit="Laporan" icon={Activity} color="yellow" trend={trend} loading={loading} />
      ...
      <div className="grid grid-cols-1 gap-5">
        <ChartWrapper
          title={tab === 'monthly' ? 'LBKB Bulanan' : 'LBKB Kumulatif'}
          subtitle={`Target vs Realisasi (Laporan) · ${filters.year}`}
      ...
      <div className="card p-5">
        <h3 className="section-title mb-4">
          Detail Data LBKB {tab === 'monthly' ? 'Bulanan' : 'Kumulatif'} (Laporan)
        </h3>
