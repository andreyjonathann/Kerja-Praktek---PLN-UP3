      ...
      const fetchNiagaData = async () => {
        ...
        try {
          ...
          const mapped = res.data.map(item => ({
            ...
            realisasi: item.data_realisasi?.pelunasan_real ?? null
          }));
          setNiagaData(mapped);
        } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const payload = {
        ...
        pelunasan_real: data.pelunasan_real ? parseFloat(data.pelunasan_real) : 0
      };
  ...
  return (
    <div className="bg-slate-50 min-h-screen w-full flex flex-col gap-6 animate-fade-in relative pb-20">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-slate-200 py-4 px-4 md:px-8 shadow-sm">
        <div className="w-full flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
             <div className="w-12 h-12 rounded-xl bg-indigo-50 flex flex-shrink-0 items-center justify-center text-indigo-600">
               <Activity size={26} />
             </div>
             <div>
          ...
          <div className="flex items-center gap-3">
             <div style={{
               ...
               <button
                 type="button"
                 onClick={() => navigate('/niaga/pelunasan')}
                 disabled={loading}
                 style={{
                   padding: '6px 20px',
                   borderRadius: 9,
      ...
      <div className="w-full px-4 md:px-8 mt-2">
        <form id="pelunasanForm" onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-4xl mx-auto pb-10">
          ...
          {success && (
            <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 px-6 py-4 rounded-xl flex items-center gap-3 animate-fade-in shadow-sm">
              ...
              <span className="font-semibold">Data Pelunasan PRR berhasil disimpan! Mengalihkan...</span>
            </div>
          )}
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                 <div>
                   ...
                   <input
                     ...
                     className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-semibold text-slate-800"
                 ...
                 <div>
                   ...
                   <div className="relative">
                     <select
                       ...
                       className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl appearance-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-semibold text-slate-800"
                     >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative">
              ...
              <div className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-100 p-5 lg:px-8 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="font-bold text-lg text-slate-800">Realisasi Pelunasan PRR</h2>
              ...
              <div className="p-6 lg:p-8 bg-slate-50/50">
                <div className="bg-white p-5 lg:p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-indigo-300 transition-colors group">
                  <label className="text-sm font-bold text-slate-700 mb-2 block group-hover:text-indigo-600 transition-colors">
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      ...
                      step="any"
                      min="0"
                      {...register('pelunasan_real', { required: 'Nilai realisasi wajib diisi' })}
                      className="w-full p-3 border border-slate-200 rounded-r-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-bold text-slate-800"
                      placeholder="0.00"
                    />
                  </div>
                  {errors.pelunasan_real && <p className="text-red-500 text-sm mt-1.5 font-medium">{errors.pelunasan_real.message}</p>}
                </div>
              </div>
