    ...
    <div className="min-h-screen bg-slate-50 flex flex-col animate-fade-in py-12">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 640, margin: '0 auto', width: '100%', padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: '#1e293b' }}>Tambah SAIDI</h1>
        ...
        {success && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#16a34a', fontWeight: 600, fontSize: '0.86rem' }}>
            <CheckCircle size={16} /> Data SAIDI Berhasil Disimpan!
          </div>
        )}
        ...
        <form style={{ display: 'flex', flexDirection: 'column', gap: 14 }} onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center gap-2 bg-slate-50/50">
              <div className="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center"><Activity size={16} /></div>
              <h3 className="font-bold text-slate-800 text-sm tracking-wide">DETAIL KOMPONEN SAIDI</h3>
            </div>
            <div className="p-5 flex flex-col gap-3">
              ...
              {[
                { name: 'saidi_distribusi_padam_tidak_terencana', label: 'Padam Tidak Terencana', icon: <Zap size={15} />, bg: 'bg-blue-50 text-blue-600' },
                { name: 'saidi_distribusi_padam_terencana', label: 'Padam Terencana', icon: <Zap size={15} />, bg: 'bg-blue-50 text-blue-600' },
                { name: 'saidi_distribusi_bencana_alam', label: 'Bencana Alam', icon: <Zap size={15} />, bg: 'bg-amber-50 text-amber-600' },
              ].map(f => (
              ...
              <div className="flex items-center justify-between p-3 bg-white border border-[#f3f4f6] rounded-xl gap-4 hover:bg-slate-50 transition">
                ...
                <input readOnly={isDuplicate} type="number" step="0.0001" {...register('saidi_transmisi')} className={fieldInputClass} placeholder="-" />
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-white border border-[#f3f4f6] rounded-xl gap-4 hover:bg-slate-50 transition">
                ...
                <input readOnly={isDuplicate} type="number" step="0.0001" {...register('saidi_pembangkit')} className={fieldInputClass} placeholder="-" />
              </div>
            </div>
          </div>
