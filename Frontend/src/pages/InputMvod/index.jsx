...
export default function InputMvodPage() {
  ...
  const [form, setForm] = useState({ tahun: currentYear, bulan: '', tipe_rct: '', menit: '', kali: '' })

  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.tahun || !form.bulan || !form.tipe_rct || form.menit === '' || form.kali === '') return alert('Semua field wajib diisi!')
    if (Number(form.menit) < 0 || Number(form.kali) < 0) return alert('Angka tidak boleh negatif!')
    ...
    try {
      const payload = {
        ...
        total_lama_padam_jam: Number(form.menit) / 60, // konversi menit ke jam
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between p-3 bg-white border border-[#f3f4f6] rounded-xl gap-4 hover:bg-slate-50 transition">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <label className="font-semibold text-slate-700 text-[13px]">Total Lama Padam (Menit) <span className="text-rose-500">*</span></label>
                </div>
                <div className="w-[140px]">
                  <input type="number" step="0.01" min="0" className={fieldInputClass} placeholder="0" value={form.menit} onChange={e => setForm({ ...form, menit: e.target.value })} required />
                </div>
              </div>
